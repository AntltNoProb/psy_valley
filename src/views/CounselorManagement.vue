<template>
    <div style="padding: 10px">
            ...
            <el-table-column label="操作">
                <template #default="scope">
                    <el-button type="success" size="small" style="margin-left: 10px"
                        @click="handleEditInformation(scope.row)">修改</el-button>
                    <el-button type="primary" size="small" style="margin-left: 10px"
                        @click="handleEditSchedule(scope.row)">排班</el-button>
                    <el-popconfirm size="small" title="确认要操作吗?"
                        @confirm="HandleUpdate(scope.row.status, scope.row.username)">
                        <template #reference>
                            <el-button type="danger" style="margin-left: 10px" size="small"
                                v-if="scope.row.status == 'normal'">禁用</el-button>
                            <el-button type="primary" style="margin-left: 10px" size="small"
                                v-show="scope.row.status == 'banned'">解禁</el-button>
                        </template>
                    </el-popconfirm>
                </template>
            </el-table-column>
        ...
        <div>
            <el-pagination v-model:currentPage="currentPage" :page-size="10" layout="prev, pager, next, jumper"
                :total="total" @current-change="handleCurrentChange" background>
            </el-pagination>
